...
@author: agentimis1
"""
#This dataset includes catogrical variables
#Feature importance plot (将 dummy 变量归到原始变量或者包括dummy后的全部变量)
...
Ex1 = pd.read_csv(main_dir+'/Data/Rice_2024.csv')
...
# 保存为 CSV
dtypes_df.to_csv(main_dir + '/Results/Rice_2024_dtypes.csv', index=False)
...
#%%# feasture importance, dummy variable归总回原始变量层级
import matplotlib.pyplot as plt


# 获取特征重要性
importances = regressor.feature_importances_
feature_names = np.array(Ex1feat.columns[1:])  # 排除掉 'Yield'，只保留自变量名称

# 读取原始数据的分类变量（不包含 'Yield'）
categorical_vars = Ex1.select_dtypes(include=['object']).columns.tolist() #.columns.tolist() 提取这些列的名称，并转换成列表。

# 创建映射，将 dummy 变量归到原始变量
feature_mapping = {}
for feature in feature_names:
    for cat_var in categorical_vars:
        if feature.startswith(cat_var):  # 如果变量名前缀匹配某个类别变量
            feature_mapping[feature] = cat_var
            break
    else:
        feature_mapping[feature] = feature  # 若不是 dummy 变量，则直接使用原始变量名

# 归总变量重要性
importance_df = pd.DataFrame({'Feature': feature_names, 'Importance': importances})
importance_df['Original Variable'] = importance_df['Feature'].map(feature_mapping)
aggregated_importance = importance_df.groupby('Original Variable')['Importance'].sum()
...
plt.barh(aggregated_importance.index, aggregated_importance.values, color='skyblue')
plt.xlabel("Aggregated Feature Importance Score")
plt.ylabel("Original Variables")
plt.title("Aggregated Feature Importance in Random Forest")
plt.gca().invert_yaxis()  # 让重要性最高的变量排在最上方
plt.show()

#%%#%%#%% Show relative variable importance in RF 直接使用转换后的变量（即所有变量，包括dummy variables）
import matplotlib.pyplot as plt
